...
def ifisnii(epi):
    ...
    if epi[-3:]=='nii':
        print "Input EPI is .nii !"
        output = True
    else:
        print "Input EPI is not .nii !"
        output = False
    
    return output

def ifisniigz(epi):
    """ Function to determine if input epi file is a gzipped nifti file. 
    """
    if epi[-6:]=='nii.gz':
        print "Input EPI is .nii.gz !"
        output=True
    else:
        print "Input EPI is not  .nii.gz !"
        output = False
...
def define_epifilenames(epipath):
    """ Function to define filenames for input epi, smoothed epi, and filtered epi. This function assumes that input epi must be a .nii or .nii.gz file. """

    filenames = dict()
    filenames['epi']=epipath.split('/')[-1] # This gets rid of the path so only has the epi name
    #epidir=params.epi[i].strip(filenames['epi']) # This removes epi from the entire path to only keep the path
    epidir=epipath.rstrip(filenames['epi']) # RW had this as rstrip (I think to fix a bug with my filenames). Does this break others' code?
    if ifisnii(epipath):
    ...
    elif ifisniigz(epipath): # sometimes the files have been gzipped (to keep support@cirl happy :) )
        epibase=filenames['epi'][0:-7] # This gets rid of the suffix (.nii.gz)    
...
def scrub_data(doscrub,prev_step_file):
    ...
    #### inputs: censor file? motion file? from afni? fsl?
    #### method: remove indiv timepoints? removed adjacent timepoints?
    ####         individual stick regressors at each motion spike?
    ####         scrub before filtering (requires interpolation) and then censor before corr? (Power)
...
def run_correlations(configout,maskdir,maskdir_masked,atlas,roilist,maskedroi_prop,mindist,s,epidir,prev_output_file,timeslen,start,stop,roi2roidir,timesdir,anatomdist_dir,filenames,project):
    ...
    for count,r in enumerate(roilist):
        ...
        if roivol==0 or roivol_masked/roivol < maskedroi_prop:
            ...
            print 'ROI %03d missing > %.2f of voxels, exclude from analysis!!' %( r, 1 - maskedroi_prop )
...
params[['cond','block']] = params[['cond','block']].astype(int) # final type int [RW 9-23-14: removed 'subid']

# If 9th parameter is passed, run just that row from the params file, otherwise, run all the non-commented rows
if len(sys.argv) > 9: 
    # run the ith subject
    ...
    stop  = len(params.index)   # end at the last row
...
for i in params.index[start:stop]: 
