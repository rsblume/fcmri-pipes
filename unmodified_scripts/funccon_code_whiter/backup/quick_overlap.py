...
def define_epifilenames(epipath):
    ...
    if ifisnii(params.epi[i]):
        ...
        epidir=params.epi[i].rstrip(filenames['epi']) # This removes epi chars from the end of the path only keep the path
...
def run_correlations(maskdir,maskdir_masked,atlas,roilist,maskedroi_prop,mindist,s,epidir,prev_output_file,timeslen,start,stop,roi2roidir,timesdir,filenames,project):
    ...
    for count,r in enumerate(roilist):
        ...
        #print 
        this_roi_prop = roivol_masked/roivol 
        if this_roi_prop < maskedroi_prop:
            # Not enough of ROI left after masking; create a timeseries of nans
            print 'ROI %03d missing %s of voxels, exclude from analysis!!' %(r,this_roi_prop)
            these_missing_rois.append(r)

    if len(these_missing_rois) > 0:
        print str(count_missing_rois) + " of " + str(count+1)
        print these_missing_rois
...
dosmooth = 'alreadysmooth'
dofilter = 'alreadyfilter'
doscrub = 'noscrub'
...
maskedroi_prop=.20 # Minimum proportion of voxels in masked ROI as compared to unmasked ROI
